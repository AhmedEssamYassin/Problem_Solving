...
Unary function that accepts an element in the range as argument, and returns a value convertible to bool. The value returned indicates whether the element is to be chosen or not (if true, it is chosen).
...
--> Copies n elements starting from l1 to overwrite elements starting from l2

copy_if(l1,r1,l2,pred_function)			Copies certain elements of range (function template)
-->Copies as many as possible elements whose pred_function returns true from [l1,r1) overwrite elements starting from l2
**Doesn't move from l2 to l2+1 until it meets an element whose pred_function returns true
-->Returns An iterator pointing to the element that follows the last element written in the result sequence.

copy_backward							Copies range of elements backward (function template)
move									Moves range of elements (function template)
...
int main() 
{
    ...
    copy_if(Myints,Myints+7,V.begin(),[] (int i){return Isprime(i);});
    for(int i:V)
    cout<<i<<" ";
    cout<<endl;
    return 0;
