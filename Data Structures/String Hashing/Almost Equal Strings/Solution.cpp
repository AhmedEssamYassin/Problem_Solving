...
int main()
{
	...
	while (t--)
	{
		...
		while (Q--)
		{
			...
			i--, j--, k--, l--;
			bool flag = false;
			int L = 0, R = min(j - i, l - k);
			int idx = 0;
			while (L <= R)
			{
				int mid = ((L + R) >> 1);
				if (strHash.get(i + L, i + mid) == strHash.get(k + L, k + mid))
					L = mid + 1;
				else
					idx = mid, R = mid - 1;
			}
			flag |= (strHash.get(i, i + idx - 1) == strHash.get(k, k + idx - 1) && strHash.get(i + idx + 1, j) == strHash.get(k + idx + 1, l));
			if (strHash.get(i, j) == strHash.get(k, l) || flag)
				cout << "SIMILAR\n";
			else
				cout << "DIFFERENT\n";
		}
