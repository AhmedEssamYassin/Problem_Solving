...
bool isOk(const HashRange &hashStr, const Hash &prefHash, int len, int N)
{
	int L{1}, R = len;
	while (R < N - 1)
	{
		if (hashStr.get(L, R) == prefHash)
			return true;
		L++, R++;
	}
	return false;
...
int main()
{
	...
	while (t--)
	{
		...
		{
			if (hashStr.get(0, L) == hashStr.get(R, str.length() - 1))
				len.push_back(L + 1);
			L++, R--;
		}
		L = 0, R = len.size() - 1;
		...
		{
			int mid = ((L + R) >> 1);
			if (isOk(hashStr, hashStr.get(0, len[mid] - 1), len[mid], str.length()))
				Length = len[mid], L = mid + 1;
			else
				R = mid - 1;
		}
		if (Length > 0)
			cout << str.substr(0, Length);
		else
			cout << "Just a legend";
	}
	return 0;
