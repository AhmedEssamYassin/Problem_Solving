...
int main()
{
	...
	while (t--)
	{
		...
		for (int i{}; i < 26; i++)
		{
			if (freq[i] == N || freq[i] == N - 1)
				return cout << "Impossible", 0;
		}
		pre(N);
		Hash hashStr = str;
		HashRange hashStrRange = str;
		for (int i{1}; i < N; i++)
		{
			Hash cur = hashStrRange.get(i, N - 1) + hashStrRange.get(0, i - 1);
			Hash revCur = hashStrRange.inv(0, i - 1) + hashStrRange.inv(i, N - 1);
			if (cur != hashStr && cur == revCur)
				return cout << 1, 0;
		}
		cout << 2;
	}
