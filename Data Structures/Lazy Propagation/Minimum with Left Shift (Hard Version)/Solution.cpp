...
public:
	segmentTree(const vector<ll> &arr)
	{
		...
		lazy = vector<LazyNode>(2 * size, INF);
	...
	pair<ll, ll> query(int left, int right)
	{
		...
		*/
		return {ans.sum, ans.weightedSum - left * ans.sum};
	}
	...
	while (t--)
	{
		...
		while (M--)
		{
			...
			else
			{
				...
				if (k == 0)
				{
					cout << segTree.query(0, N - 1).second << endl;
					continue;
				}
				auto [sum, wSum] = segTree.query(0, k - 1);
				cout << segTree.query(k, N - 1).second + wSum + (N - k) * sum << endl;
			}
		}
	}
	return 0;
