...
public:
	...
	void update()
	{
		size = 1;
		subtreeSum = value;
		// Update max values considering current node
		firstMax = value;
		secondMax = nINF;
		countFirstMax = 1;
		countSecondMax = 0;
		if (left != nullptr)
		{
			...
			// Update max values considering left child
			if (left->firstMax > firstMax)
			{
				secondMax = firstMax;
				countSecondMax = countFirstMax;
				firstMax = left->firstMax;
				countFirstMax = left->countFirstMax;
			}
			else if (left->firstMax == firstMax)
				countFirstMax += left->countFirstMax;
			else if (left->firstMax > secondMax)
			{
				secondMax = left->firstMax;
				countSecondMax = left->countFirstMax;
			}
			if (left->secondMax > secondMax && left->secondMax != firstMax)
			{
				secondMax = left->secondMax;
				countSecondMax = left->countSecondMax;
			}
		}
		if (right != nullptr)
		{
			...
			// Update max values considering right child
			if (right->firstMax > firstMax)
			{
				secondMax = firstMax;
				countSecondMax = countFirstMax;
				firstMax = right->firstMax;
				countFirstMax = right->countFirstMax;
			}
			else if (right->firstMax == firstMax)
				countFirstMax += right->countFirstMax;
			else if (right->firstMax > secondMax)
			{
				secondMax = right->firstMax;
				countSecondMax = right->countFirstMax;
			}
			if (right->secondMax > secondMax && right->secondMax != firstMax)
			{
				secondMax = right->secondMax;
				countSecondMax = right->countSecondMax;
			}
		}
	}
	...
	inline void applyUpdate(ll x, bool isBeats = false)
	{
		if (isBeats)
		{
			if (x >= firstMax) // Don't need to do anything
				return;
			if (x > secondMax) // but x < firstMax, that means we will ONLY affect the elements = firstMax
			{
				BeatsProp = true;
				subtreeSum -= (firstMax - x) * countFirstMax;
				firstMax = x;
				lazyMin = x;
				value = min(value, x);
			}
			else // if (x <= secondMax), we must go down to update children
			{
				push();
				value = min(value, x);
				if (left != nullptr)
					left->applyUpdate(x, true);
				if (right != nullptr)
					right->applyUpdate(x, true);
				update();
				return;
			}
		}
		else // ONLY lazy addition
		{
			value += x;
			lazyAddValue += x;
			subtreeSum += x * size;
	...
	void push()
	{
		...
		{
			if (left != nullptr)
				left->applyUpdate(lazyMin, true);
			if (right != nullptr)
				right->applyUpdate(lazyMin, true);
			BeatsProp = false;
		}
	}
...
inline void splayBeats(SplayTree *x)
{
	x->applyUpdate(height, true);
}
...
int main()
{
	...
	while (t--)
	{
		...
		for (auto &x : vc)
			cin >> x;
		SplayTree *spTree = new SplayTree(vc[0]);
		for (int i{1}; i < N; i++)
			spTree = SplayTree::merge(spTree, new SplayTree(vc[i]));
		while (Q--)
		{
			ll query, L, R;
			cin >> query >> L >> R;
			...
			{
				cin >> height;
				apply<splayBeats>(spTree, L, R);
			}
			else if (query == 2)
			{
				auto [leftPart, middle] = SplayTree::split(spTree, L - 1);
				auto [seg, rightPart] = SplayTree::split(middle, R - L + 1);
				spTree = SplayTree::merge(SplayTree::merge(leftPart, rightPart), seg);
			}
			else if (query == 3)
			{
				cin >> increment;
				apply<lazyAdd>(spTree, L, R);
			}
			spTree->splay();
			cout << spTree->subtreeSum << endl;
		}
	}
	return 0;
}