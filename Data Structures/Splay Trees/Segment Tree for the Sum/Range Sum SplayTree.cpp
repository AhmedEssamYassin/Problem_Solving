...
int main()
{
	...
	while (M--)
	{
		...
		{
			cin >> i >> v;
			// Split into three parts: left of index, index, right of index
			auto [leftTree, rightPart] = SplayTree::split(spTree, i);
			auto [target, rightTree] = SplayTree::split(rightPart, 1);

			// Create new node with updated value
			SplayTree *newNode = new SplayTree(v);
			...
			// Merge everything back together
			spTree = SplayTree::merge(leftTree, newNode);
			spTree = SplayTree::merge(spTree, rightTree);
			spTree->splay();
		}
		else
		{
			cin >> L >> R;
			// Split into three parts: left of range, range, right of range
			auto [leftTree, midRight] = SplayTree::split(spTree, L);
			auto [rangeTree, rightTree] = SplayTree::split(midRight, R - L);

			// Get sum of range
			ll sum = rangeTree ? rangeTree->subtreeSum : 0;

			// Merge everything back
			spTree = SplayTree::merge(leftTree, rangeTree);
			spTree = SplayTree::merge(spTree, rightTree);
			cout << sum << endl;
		}
	}
