...
int main()
{
	...
	while (t--)
	{
		...
		vector<vector<XORBasis>> up_path(N, vector<XORBasis>(LOG));

		for (const int &x : lca.tour_list) // traversing nodes visited in the Eulerian tour
		{
			up_path[x][0].insertVector(vc[x]);

			for (int level = 1; level < LOG; level++)
			{
				up_path[x][level] = up_path[x][level - 1];
				int anc = lca.get_kth_ancestor(x, 1 << (level - 1));

				if (anc >= 0)
					up_path[x][level] += (up_path[anc][level - 1]);
			}
		}
		int Q;
		cin >> Q;
		...
		for (int q = 0; q < Q; q++)
		{
			...
			y--;
			XORBasis path;
			int anc = lca.get_LCA(x, y);

			for (int level = 0; level < LOG; level++)
			{
				if ((lca.depth[x] - lca.depth[anc]) >> level & 1)
				{
					path += (up_path[x][level]);
					x = lca.get_kth_ancestor(x, 1 << level);
				}

				if ((lca.depth[y] - lca.depth[anc]) >> level & 1)
				{
					path += (up_path[y][level]);
					y = lca.get_kth_ancestor(y, 1 << level);
				}
			}

			path.insertVector(vc[anc]);
			cout << (path.canRepresent(k) ? "YES" : "NO") << '\n';
		}
	}
	return 0;
}