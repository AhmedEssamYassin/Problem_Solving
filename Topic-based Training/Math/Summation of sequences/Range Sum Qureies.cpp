...
{
	// ll Y = Get_No_Of_Terms_Of_Multiples(N, X);
	ll Y = (N / X);
	return (X * Y * (Y + 1) / 2);
}
...
int main()
{
	...
	*/
	...
	while (t--)
	{
		...
		Start++;
		End--;
		cout << (Sum_Of_ODD(End) - Sum_Of_ODD(Start - 1)) << endl;
	}
	/*
	ll L, R;
	cin >> L >> R;
	ll Start = min(L, R);
	ll End = max(L, R);
	cout << Sum_From_1_To_N(End) - Sum_From_1_To_N(Start - 1) << endl;
	cout << Sum_Of_EVEN(End) - Sum_Of_EVEN(Start - 1) << endl;
	cout << Sum_Of_ODD(End) - Sum_Of_ODD(Start - 1);
	*/
	/*
