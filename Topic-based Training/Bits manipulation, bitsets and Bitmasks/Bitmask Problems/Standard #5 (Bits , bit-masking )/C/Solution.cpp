...
int main()
{
	...
	while (Q--)
	{
		...
		if (Query == 1)
		{
			// Change the value of the rightmost cleared bit to 1
			// (N + 1) flips all bits after the rightmost cleared bit of N, including the rightmost cleared bit
			// 11011101(0)1
			// OR
			// 11011101(1)0
			// =
			// 11011101(1)1
			Set_last_cleared_bit(N);
			// N |= (N + 1);
		}
		else if (Query == 2)
		{
			// Change the value of the rightmost set bit to 0
			// (N - 1) flips all bits after the rightmost set bit of N, including the rightmost set bit
			// 1101110(1)00
			// AND
			// 1101110(0)00
		...
		{
			// Change ALL bits (from index 0 until first bit equal to 1) to 1, if N equal 0,then N will be −1
			// 0011110100
			// OR
			// 0000000011
			// 0011110111
			// Clear_all_trailing_zeros()
			ll X = (Get_rightmost_set_bit_number(N) - 1);
			if (N == 0)
				N = -1;
			else
				N |= X;
		...
		{
			// Change ALL bits (from index 0 until first bit equal to 0) to 0
			// 1110110111
			// AND
			// 1110111000
			// 1110110000
			Clear_all_trailing_ones(N);
		}
