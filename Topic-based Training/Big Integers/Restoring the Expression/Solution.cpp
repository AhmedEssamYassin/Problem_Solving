...
bigint bigPow(bigint N, bigint power)
{
	bigint res = 1;
	while (power > 0)
	{
...
ll b1 = 10;
vector<ll> Pb1, Hash;
void pre(ll maxSize, const string &str)
{
	Pb1 = Hash = vector<ll>(maxSize + 1, 1);
	Hash[0] = (str[0] - '0');
	for (int i = 1; i <= maxSize; i++)
	{
		Hash[i] = (mult64(Hash[i - 1], b1) + (str[i] - '0')) % mod;
...
ll get(int l, int r)
{
	...
	if (!l)
		return Hash[r];
	return (Hash[r] - mult64(Hash[l - 1], Pb1[r - l + 1]) + mod) % mod;
}
...
bool tryLengths(const string &str, int len1, int len2, int len3)
{
	int N = str.length();
	if ((get(0, len1 - 1) + get(len1, len1 + len2 - 1)) % mod == get(len1 + len2, N - 1))
	{
		// substr(pos, length)
		string a = str.substr(0, len1);
		string b = str.substr(len1, len2);
		string c = str.substr(N - len3);

		if (valid(a) && valid(b) && valid(c) && bigint(a) + bigint(b) == bigint(c)) // For a deterministic solution use bigint
		{
			cout << a << '+' << b << '=' << c << '\n';
			return true;
		}
...
int main()
{
	...
	while (t--)
	{
		...
		*/
		for (int len3 = 1; len3 <= N - 2; len3++)
		{
			// Try len1
			{
				int len1 = len3;
				int len2 = N - len1 - len3;
				if (tryLengths(str, len1, len2, len3))
					return 0;
				len1 = len3 - 1;
				len2 = N - len1 - len3;
				if (tryLengths(str, len1, len2, len3))
