...
private:
	...
	{
		Node res;
		res.maximum = max(leftNode.maximum, rightNode.maximum);
		return res;
	}
	...
	void push(int left, int right, int node)
	{
		if (lazy[node].value == 0)
			return;
		// Propagate the value
		seg[node] = (seg[node].maximum + lazy[node].value);
	...
	Node query(int left, int right, int node, int x, int pos)
	{
		...
		push(left, right, node);
		if (seg[node].maximum < x || right < pos)
			return -1;
		if (left == right)
			return {left};

		Node traversingLeft = query(left, mid, L, x, pos);
		if (traversingLeft.maximum == -1)
