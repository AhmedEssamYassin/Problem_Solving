...
private:
	...
	struct LazyNode
	{
		...
		friend LazyNode max(const LazyNode &LHS, const LazyNode &RHS)
		{
	...
	void push(int left, int right, int node)
	{
		...
		if (lazy[node].value == -1)
			return;
		seg[node] = max(seg[node].value, lazy[node].value);
	...
	void update(int left, int right, int node, int leftQuery, int rightQuery, const ll &val)
	{
		...
		if (left >= leftQuery && right <= rightQuery)
		{
			...
			lazy[node] = max(lazy[node].value, val);
...
public:
	segmentTree(const vector<ll> &arr)
	{
		...
		lazy = vector<LazyNode>(2 * size, -1); // Assign it to a Not-possible / Neutral value
