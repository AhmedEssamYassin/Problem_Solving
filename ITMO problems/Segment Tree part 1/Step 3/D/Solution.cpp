...
int main()
{
	...
	for (int i{}; i < 2 * N; i++)
	{
		...
		if (pos[X] == -1) // First time to see (the left of this segment)
			pos[X] = i, segTree.update(i, 1);

		else // Second time to see (the right of this segment)
		{
			ans[X] = segTree.query(pos[X] + 1, i);

			// Updating the left to be -1 so that if it appears with the right in the same segment they cancel each other
			segTree.update(pos[X], -1);

			// Updating right to be 1
			segTree.update(i, 1);
		}
	}
	for (int i{1}; i <= N; i++)
